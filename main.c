...
struct editorConfig
{
  ...
  int numrows;
  erow *row;
  int dirty; // true if file is modified since saving or opening
  char *filename;
  char statusmsg[80];
...
void editorSave()
{
  ...
  // 0644 standard permissions for opening file
  // gives read/write to file owner and read to all else
...
{
  abAppend(ab, "\x1b[7m", 4); // invert text color
  ...
  abAppend(ab, "\x1b[m", 3); // set text color back to normal
  abAppend(ab, "\r\n", 2);   // new line for status message
}
...
void editorRefreshScreen()
{
  ...
  // "\x1b" = escape
  abAppend(&ab, "\x1b[?25l", 6); // hide curosr
